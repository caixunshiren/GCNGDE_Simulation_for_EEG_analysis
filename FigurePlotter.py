...
class FigurePlotter():
    ...
    def sim_progression(self, IM, dm, Araw, model_name):
        def sequential_plot(lis, DIR, names=None):
            plt.clf()
            fig, ax = plt.subplots(nrows=1 if len(lis) <= 4 else int(len(lis)/4) if len(lis)%4 == 0 else int(len(lis)/4) + 1, ncols=4, figsize=(20,20),constrained_layout=True)
                ...
                for col in ax:
                    if i < len(lis):
                        ...
                        if names is not None:
                            ...
                            col.title.set_size(30)

                        col.axis('off')
                        i += 1
                    else:
                        im = col.imshow(np.zeros(lis[0].shape), cmap='bwr')
                        col.axis('off')
                        i += 1
            else:
                for row in ax:
    ...
    def training_curve_MLP(self, IMs, labels, cap=None):
        for i,IM in enumerate(IMs):
            ...
            plt.plot(range(len(val_loss)), val_loss, label=labels[i*2])
            records = IM.MLPcheckpointGDE['record']
            if cap is None:
                cap = len(records['val_loses'])
            val_loss = records['val_loses'][:cap]
            plt.plot(range(len(val_loss)), val_loss, label=labels[i*2+1])
        ...
        for i,IM in enumerate(IMs):
            ...
            plt.plot(range(len(val_auc)), val_auc, label=labels[i*2])
            records = IM.MLPcheckpointGDE['record']
            if cap is None:
                cap = len(records['val_auc'])
            val_auc = records['val_auc'][:cap]
            plt.plot(range(len(val_auc)), val_auc, label=labels[i*2+1])
