...
from graphsage.utils import Graph_complement
import numpy as np
...
class EdgeMinibatchIterator(object):
    ...
    def __init__(self, G, id2idx, 
        ...
        if context_pairs is None:
            ...
            edges = context_pairs
        if(len(edges)>1):
            self.train_edges = self.val_edges = self.edges = np.random.permutation(edges)
        else:
            self.train_edges = self.val_edges = self.edges = edges
...
#         print(len([n for n in G.nodes() if G.node[n]['test'] or G.node[n]['val']]), 'test nodes')
        ...
        self.agg_batch_Z1, self.agg_batch_Z2 = self.nodes_from_edges(self.G.edges())
        self.agg_batch_Z3, self.agg_batch_Z4 = self.nodes_from_edges(self.Gneg.edges())
    ...
    def construct_adj(self):
        adj = len(self.id2idx)*np.ones((len(self.id2idx)+1, self.max_degree), dtype=np.int32)# changed, added , dtype=np.int32
        ...
        for nodeid in self.G.nodes():
            ...
            neighbors = np.array([neighbor # changed self.id2idx[neighbor] to neighbor
                for neighbor in self.G.neighbors(nodeid)
                if (not self.G[nodeid][neighbor]['train_removed'])])
            deg[nodeid] = len(neighbors) # changed self.id2idx[nodeid] to nodeid
            if len(neighbors) == 0:
                continue
            ...
            adj[nodeid, :] = neighbors # changed self.id2idx[nodeid] to nodeid
        return adj, deg
    ...
    def end(self):
        ...
        #return self.end_elem_num>= len(self.train_edges)
    
    def nodes_from_edges(self, batch_edges):
        batch1 = []
        batch2 = []
        for node1, node2 in batch_edges:
            batch1.append(node1) # changed self.id2idx[node1] to node1
            batch2.append(node2) # changed self.id2idx[node2] to node2
        return batch1, batch2
    
    def batch_feed_dict(self, batch_edges):
        
        batch1, batch2 = self.nodes_from_edges(batch_edges)
        feed_dict = dict()
        feed_dict.update({self.placeholders['batch_size'] : len(batch_edges)}) # self.batch_size
        feed_dict.update({self.placeholders['batch1']: batch1})
        feed_dict.update({self.placeholders['batch2']: batch2})
        feed_dict.update({self.placeholders['dropout']: FLAGS.dropout})
        feed_dict.update({self.placeholders['all_nodes']: [node for node in self.G.nodes()]})
            
        feed_dict.update({self.placeholders['agg_batch_Z1']: self.agg_batch_Z1})
        feed_dict.update({self.placeholders['agg_batch_Z2']: self.agg_batch_Z2})
        feed_dict.update({self.placeholders['agg_batch_Z3']: self.agg_batch_Z3})
        feed_dict.update({self.placeholders['agg_batch_Z4']: self.agg_batch_Z4})
        
        return feed_dict
    
    
    def graph_update(self, new_adj_info, new_batch_edges):
        
        #start_idx = self.batch_num * self.batch_size
        start_idx = self.start_elem_num
        end_idx = self.end_elem_num
        
        self.train_edges.extend(new_batch_edges) # how to not append the existing ones??????????
        self.val_edges.extend(new_batch_edges)
        self.edges.extend(new_batch_edges)
        
        #self.G.edges() = self.edges
        self.G.neighbors
        self.max_degree  
        self.adj, self.deg = self.construct_adj()
        self.test_adj = self.construct_test_adj() 
        
    def feed_dict_negbatch(self, batch1):  
        batch2 = []
        for node1 in batch1:
            flagg = False
            for node2 in self.G.nodes():
                if((node1,node2) in self.batch_edges):
                    flagg = True
                    batch2.append(node2) 
                    break
#             if(not flagg):
#                 batch2.append(node2)
    ...
    def next_minibatch_feed_dict(self):
        if(len(self.train_edges)>1):
            start_idx = self.batch_num * self.batch_size
            #start_idx = self.start_elem_num = self.end_elem_num
            self.batch_num += 1
            end_idx = min(start_idx + self.batch_size, len(self.train_edges))
            #self.end_elem_num = end_idx
            self.batch_edges = self.train_edges[start_idx : end_idx]
        else:
            self.batch_edges = self.train_edges
        return self.batch_feed_dict(self.batch_edges)
    ...
    def val_feed_dict(self, size=None):
        ...
        if((size is None) or (len(edge_list)==1)):
