...
GraphLCore = namedtuple('GraphLCore', ['model_size', 'num_nodes', 'dim', 'fixed_params','aggregator_type',\
                                       ...
                                       'A_regularizer'])
"""
------------------ Properties of graph learning (i.e. GNN + similarity matrix generation) ----------------------- 
    model_size: default is 'small' for graphs of sizes less than O(10^5)
    num_nodes: number of graph nodes, vary per patient, refer to Table 
    dim: the dimension of the initial feature vector, i.e. D_0
    fixed_params
    aggregator_type: the string corresponding to the function used for the aggregation process in the GNN, default is 'mean'
    concat: boolean value 
    num_layers: number of aggregation layers (i.e. K), default is 1 -> !Caution! when changing it, might be partially implemented.
    conv_sizes: # of features (per sample per node) that belong to each frequency, only in frequency domain 
    side_adj_mat: default is a binary array the is fed to GNN (i.s. A in eq. (44) in the paper), 
                          if None, A is updated at each SGD batch update
    A_val_string: the string shows how the GNN initial graph structure (i.e. A) is computed 
                                             from all the training samples. 
                                             default is 'fixedA_invCov' i.e. using when is calculated from the inverse Covariance matrix
    coordinate_gradient: default is False, if True "and" if side_adj_mat is None, A is update at every SGD batch update
    projected_gradient: default is True
    dimArray: the array of elements each showing the dimension of the signal ..  
    varType: showing the mode that the GNN weights (i.e. U and b in eq (14) in the paper )
                                                              and \Theta (eq (36) in the paper) are defined
                                        default vary for each patient (refer to Table 3 in the paper)
                                        'scalar' refer to eq (46) in the paper
                                        'full' refer to mode (1) in the appendix of the paper
    A_regularizer: default is None, i.e. the GNN initial graph structure (i.e. A) is not being regularized, 
                                              otherwise it is a positive number showing the coefficient for the A regularizer 
""" 
...
class train_test(Task):
    ...
    def in_W_graphL_run(self, hyperparams, in_train_measures=None, in_valid_measures=None, only_training=False):
        ...
        """
        GNN + Similarity Matrix Generation + Classification: Training and Testing for a fixed set of hyper-parameters
        hyperparams: contains  epochs: number of epochs for the training with stochastic gradient descent (SGD),
                               learning_rate: the learning rate of the stochastic gradient descent (SGD)
                               num_trees: the number of trees to be used in the random forest classifier
                               batch_size: the batch-size used for the stochastic gradient descent (SGD)
                               eval_thresh_func: the function used for the aggregation process in the GNN,
                                                 default is np.mean
                               aggregator_type: the string corresponding to the function used for the aggregation process in the GNN, 
                                                default is 'mean'
                               A_val_string: the string shows how the GNN initial graph structure (i.e. A) is computed 
                                             from all the training samples. 
                                             default is 'fixedA_invCov' i.e. using when is calculated from the inverse Covariance matrix
                               feature_normalize: boolean value showing whether features are normalized before being fed to the classifier
                               varType: showing the mode that the GNN weights (i.e. U and b in eq (14) in the paper )
                                                              and \Theta (eq (36) in the paper) are defined
                                        default vary for each patient (refer to Table 3 in the paper)
                                        'scalar' refer to eq (46) in the paper
                               ...
                               A_regularizer: default is None, i.e. the GNN initial graph structure (i.e. A) is not being regularized, 
                                              otherwise it is a positive number showing the coefficient for the A regularizer 
        in_train_measures: a list that saves the performance evaluation of the training data for each set of hyper-parameters
        in_valid_measures: a list that saves the performance evaluation of the validation data for each set of hyper-parameters
        only_training : Boolean value, default is True when the model if only being trained, without testing at this point
        """
        ...
        if(True):
            ...
            classifier_properties = RF_properties(loss_type='softmax', num_trees=num_trees, \
        ...
        """
        ------------------------ setting the properties of graph learning (i.e. GNN + similarity matrix generation) ----------------------- 
        """    
        graphL_core = GraphLCore(model_size='small', num_nodes=self.matlab_load_core.num_nodes, dim=np.prod(self.dimArray), \
                                fixed_params=False, aggregator_type=aggregator_type,\
        ...
        """
        ------------------------ setting the properties of the classifier ----------------------- 
        """  
        ...
        """
            Class comprised of the functions implementing GNN and similarity matrix generation (i.e. the features extracted)
        """
        ...
        """
            training the GNN and similarity matrix generation
        """
        ...
        if(not self.task_core.supervised):
            """
                training the classifier disjointedly from the Graph learning process
            """
            prob_hat_train = feature_extraction.train_disjointed_classifier(self.X_train[self.train_idx,...], \
                                                                self.y_train[self.train_idx], self.task_core.target, printing_flag=True)
        if(not only_training):
            feature_extraction.printing(self.task_core.adj_calc_mode)
        """
            perform the whole GNN + similarity matrix generation + classifier using the previously trained parameters ...
        """
    ...
    def run(self):
        """
        Main inner function to apply training and testing on the data of each patient
        """
        ...
        if('W_graphL' in self.task_core.feature_mode):
            hyperparams_arr = list(itertools.product(  self.task_core.epochs_arr, self.task_core.learning_rate_arr,\
                                                       self.task_core.num_trees_arr, self.task_core.eval_thresh_func_arr,\
                                                       self.task_core.batch_size_arr, self.task_core.aggregator_type_arr,\
                                                       self.task_core.A_val_string_arr, self.task_core.feature_normalize_arr,\
                                                       self.task_core.varType_arr, self.task_core.A_regularizer_arr))
            self.validation_flag = len(hyperparams_arr)>1
        else:
            hyperparams_arr = list(itertools.product(self.task_core.freq_bands, self.task_core.num_trees_arr))
            self.validation_flag = len(self.task_core.freq_bands)>1 or len(self.task_core.num_trees_arr)>1
            
        valSplitter = StratifiedKFold(n_splits=self.task_core.valid_split_ratio)
        load_core = LoadCore(preszr_sec=10, postszr_sec=10000, band_nums=self.task_core.band_nums)
        
        
        if('50%-50%' in self.task_core.TrainTest_mode):
            """
                Training and testing of '50%-50%' selects all the seizure samples, 
            ...
            in_train_measures = []
            hypparamCounter = 0
            for A_density_ratio in self.task_core.A_density_ratio_arr:
                ...
                if('fine' in self.task_core.TrainTest_mode):
                    ...
                    X_test = None 
                    y_test = None 
                for multiClass_ratio in self.task_core.multiClass_ratio_arr:
                    ...
                    for self.train_idx, self.valid_idx in valSplitter.split(np.arange(self.y_train.size), self.y_train):
                        ...
                        """
                        ------------------------------- Training and Validating on various hyper-parameters ---------------------------- 
                        """
                        if('W_graphL' in self.task_core.feature_mode):
            ...
            if('W_graphL' in self.task_core.feature_mode):
                hyperparams_arr = list(itertools.product(self.task_core.A_density_ratio_arr, self.task_core.epochs_arr, self.task_core.learning_rate_arr,\
                                                  self.task_core.num_trees_arr, self.task_core.eval_thresh_func_arr,\
                                                   self.task_core.batch_size_arr, self.task_core.aggregator_type_arr,\
                                                   self.task_core.A_val_string_arr, self.task_core.feature_normalize_arr,\
                                                   self.task_core.varType_arr, self.task_core.A_regularizer_arr))
    ...
    #             rank_hyperparams(itertools.product(self.task_core.epochs_arr, self.task_core.learning_rate_arr, self.task_core.num_trees_arr, self.task_core.eval_thresh_func_arr, self.task_core.batch_size_arr), valid_measures)
            """
                find the index of the hyper-parameter set that gives the best performance
            """
            best_hyper = 0
            if(self.validation_flag):
                ...
                print('    Best hyperparams are: A_density_ratio=%f, epochs = %d, learning_rate = %f, num_trees = %d, eval_thresh_func = %s, batch_size = %d, aggregator-type=%s, A_val_string=%s, feature_normalize=%s, varType=%s, A_regularizer=%s'\
            ...
            if('W_graphL' in self.task_core.feature_mode):
                ...
                """
                -------- Testing the performance of classifier using Graph Learning features (i.e. GNN+similarity matrix)--------------- 
                """
                ...
                if(X_test is not None):
                    ...
                    fine_eval_performance(y_test, prob_hat, self.task_core.sensitivity_score_target, th=valid_eval_threshold, printing_flag=True)  
                    print('********')
                elif(X_test is None):
                    ...
                    while(not classif_loadbatch.end()):
                        ...
                        coarse_eval_performance(y_true, prob_hat, self.task_core.sensitivity_score_target,\
                                                th=valid_eval_threshold, matlab_load_core=self.matlab_load_core,\
                                                mvAvg_winlen=self.task_core.mvAvg_winlen) 
        ...
        elif('24h-1h' in self.task_core.TrainTest_mode):
            """
                Training and testing of '24h-1h' is in real-time mode, i.e. samples correspond to consecutive windows. 
                ...
                for epochs, learning_rate, num_trees, eval_thresh_func, batch_size, aggregator_type, A_val_string, multiClass_ratio, feature_normalize, varType, A_regularizer in hyperparams_arr:
                    ...
                    if('W_graphL' in self.task_core.feature_mode):
                        ...
                        classification_core = ClassifCore(classifier=classifier, classifier_properties=classifier_properties,
                                                  ...
                                                  sensitivity_score_target=self.task_core.sensitivity_score_target,\
                    ...
                    print('TRAINING')
                    fine_eval_performance(in_y_train, prob_hat_train, self.task_core.sensitivity_score_target, printing_flag=True)
                    break
...
class Matlab_Load_Core():
    """
        decodes a loaded .mat file 
        returns 
            conv_sizes: # of features (per sample per node) that belong to each frequency, only in frequency domain 
            soz_ch_ids: an array of channels that are clinically considered as SOZ (i.e. Seizure onset zone)
            pre_ictal_num_win: number of windows (i.e. samples) that are labeled as 1 (i.e. same as ictal samples) but are actually pre-ictal
            window_size_sec: the duration of the chunk of iEEG signal that is cut and considered as one "sample"
            stride_sec: the duration of overlap between consecutive iEEG samples
            sel_win_nums_train:  
            clip_sizes_train  : is a 2-d array of shape 2 * #training-samples, 
                                clip_sizes[0,i] shows the sample index that the i-th training clip starts from
                                clip_sizes[1,i] shows the sample index that the i-th training clip ends with
                                each clip is a one-hour iEEG that is acquired consecutively and in real time
            clip_sizes_test   : ,,, for testing samples
            X_train: training samples of shape --- #samples * #nodes * sampleDimenstion(=sampling-frequency*window_size_sec)
            X_test : testing  samples of shape --- ... 
            y_train: training labels of shape --- #samples
...
class data_load(Task):
    """
        Class for loading data (i.e. initial features, labels, etc.)
        Task: an instance of the TaskCore class in main.py
    """    
    def EU_matlab_run(self, load_Core):
        """
        loads data features (i.e. initial time- and frequency-domain features) that has already been computed and saved using a MATLAB script 
        load_Core: an instance of the LoadCore class
        """
...
#             num_clips.append(np.array(inner_x).shape[0])
    ...
    def run(self, load_Core):
        """
            select a intended function to load data and run it 
        """
        print ('Loading data') 
