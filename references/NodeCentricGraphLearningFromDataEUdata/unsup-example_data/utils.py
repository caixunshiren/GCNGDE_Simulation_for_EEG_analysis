...
#         X = np.where(A==1, X+2, X)
        ...
        probs.append(prob)
#     pdf = @(X) X_pdf(A, Theta, U_GT, U_tilde_GT, X, num_layers);
#     samples = mhsample(start,nsamples,'logpdf',pdf,'proppdf',proppdf, 'proprnd',proprnd);
#     for counter=range(nsamples):
#         probs(counter) = X_pdf(A, Theta,U_GT, U_tilde_GT, samples{counter}, num_layers);    
