...
def X_to_W(arr, feature_mode, sampling_freq=256, freq_band=None):
    arr = np.squeeze(arr)
    if(arr.ndim > 3):
...
#         W = np.array([[[pearsonr(np.squeeze(arr[samp,row,:]),np.squeeze(arr[samp,col,:]))[1] for row in np.arange(num_nodes)] for col in np.arange(num_nodes)] for samp in np.arange(arr.shape[0])])
        W = np.array([pandas.DataFrame(data=np.squeeze(arr[samp,:,:])).T.corr().as_matrix() for samp in np.arange(arr.shape[0])]) #
        del arr
        
        
    triu_indices = np.triu_indices(W.shape[-1])
    return np.reshape(W, (W.shape[0], -1)) # np.array([W[i,triu_indices[0],triu_indices[1]] for i in np.arange(W.shape[0])]) #  

def X_flat_complex(arr):
    arr_flat = arr.reshape(arr.shape[0], -1)
    return np.concatenate((np.real(arr_flat), np.imag(arr_flat)), axis=1)
     
def multiClass_undersampling(X, y, multiClass_ratio, clip_sizes):
    if(multiClass_ratio == -1 or multiClass_ratio is None):
        return X, y , clip_sizes
#     rus = RandomUnderSampler(random_state=42)
    ...
    return X[sel_idx,...], y[sel_idx], clip_sizes # np.concatenate((X[y!=0,...], X[sel_idx,...]), axis=0), np.concatenate((y[y!=0], y[sel_idx]), axis=0)
...
class train_test(Task):
    ...
    def run(self):
        ...
        matlab_load_core.num_nodes = num_nodes
        ...
        side_adj_mat_arr = [(2*matlab_load_core.structural_inf.adj_means[0]-1)] #  (2*matlab_load_core.structural_inf.adj_means[0]-1)] # 
        ...
        multiClass_ratio_arr = [2]
        feature_normalize_arr = [False]
        varType_arr = ['full'] # 'diag_repeated', 'full_repeated', 'full'
        A_regularizer_arr = [None] # None or weight of regularization
        extra_feat_functions = None # [FFTWithTimeFreqCorrelation(1, 48, 400, 'usf')]
#         freq_bands = [[0.1, 4], [4, 8], [8, 13], [13, 30], [30, 50], [70, 100]]
        ...
        if('W_graphL' in self.task_core.feature_mode ):
            validation_flag = len(hyperparams_arr)>1
        else:
            validation_flag = len(freq_bands)>1 or len(num_trees_arr)>1
        ...
        if('50%-50%' in self.task_core.TrainTest_mode):
            ...
            for multiClass_ratio in multiClass_ratio_arr:
                X_train, y_train, matlab_load_core.clip_sizes_train = \
                            multiClass_undersampling(matlab_load_core.X_train, matlab_load_core.y_train, multiClass_ratio, matlab_load_core.clip_sizes_train)
                ...
                for train_idx, valid_idx in valSplitter.split(np.arange(y_train.size), y_train):
                    if(not validation_flag):
                        train_idx = np.arange(y_train.size)
    #                 train_idx, valid_idx = valSplitter.split(np.arange(y_train.size), y_train)[0]
        
                    if('W_graphL' in self.task_core.feature_mode):
                        
                        for epochs, learning_rate, num_trees, eval_thresh_func, batch_size, aggregator_type, A_val_string,\
                             feature_normalize, varType, A_regularizer in hyperparams_arr:
#                             try:
                                if(True):
                                ...
                                graphL_core = GraphLCore(model_size='small', num_nodes=num_nodes, dim=np.prod(dimArray), fixed_params=False, aggregator_type=aggregator_type,\
                                                    ...
                                                    dimArray=dimArray, varType=varType, A_regularizer=A_regularizer) 
                                ...
                                classification_core = ClassifCore(classifier=classifier, classifier_properties=classifier_properties,
                                                          ...
                                                          mvAvg_winlen=mvAvg_winlen,\
                                                          feature_normalize=feature_normalize,\
                                                          extra_feat_functions=extra_feat_functions)
                                
                                feature_extraction = Hybrid_Rep_Feat(graphL_core, classification_core, matlab_load_core, self.task_core, weight_losses)
            ...
            #                                                                                 out_th = np.arange(0.27, 0.32, 0.008), printing_flag=True) 
                                feature_extraction.train(X_train[train_idx,...], y_train[train_idx], self.task_core.target, printing_flag=True)
                                if(not self.task_core.supervised):
                                    prob_hat_train = feature_extraction.train_disjointed_classifier(X_train[train_idx,...], y_train[train_idx], self.task_core.target, printing_flag=True)
                                feature_extraction.printing(self.task_core.adj_calc_mode)
                                if(self.task_core.supervised):
                                    _, _, _, _, prob_hat_train, _ = feature_extraction.test(X_train[train_idx,...], y_train[train_idx], \
                                                                                                   self.task_core.target, train_test='training',\
                                                                                                   printing_flag=True, plotting_flag=Windows,\
                                                                                                   clip_sizes = matlab_load_core.clip_sizes_train,\
                                                                                                   sel_win_nums = matlab_load_core.sel_win_nums_train[train_idx])
                                if(validation_flag):
                                    _, _, _, _, prob_hat_valid, _ = feature_extraction.test(X_train[valid_idx,...], y_train[valid_idx], \
                                                                                                   self.task_core.target, train_test='validation', \
                                                                                                   printing_flag=True, plotting_flag=Windows,\
                                                                                                   clip_sizes = matlab_load_core.clip_sizes_train,\
                                                                                                   sel_win_nums = None) #matlab_load_core.sel_win_nums_train[valid_idx] 
                                ...
                                if(validation_flag):
                                    in_valid_measures.append(fine_eval_performance(y_train[valid_idx], prob_hat_valid, sensitivity_score_target, printing_flag=True))
#                             except Exception as e:
#                                 print(e)
#                                 continue
                    else:
                        
                        for freq_band, num_trees in list(itertools.product(freq_bands, num_trees_arr)):
                            clf = RandomForestClassifier(n_estimators=num_trees)
                            if('W_raw' in self.task_core.feature_mode): 
                                feature_trained = X_to_W(X_train[train_idx,...], self.task_core.feature_mode, freq_band=freq_band)
                                clf = clf.fit( feature_trained, y_train[train_idx] )
                                prob_hat_train = clf.predict_proba(feature_trained)[:, 1]
                                if(validation_flag):
                                    prob_hat_valid = clf.predict_proba(X_to_W(X_train[valid_idx,...], self.task_core.feature_mode, freq_band=freq_band))[:, 1]
...
#                 print('-------')
            ...
            valid_eval_threshold = np.arange(0.25, 0.8, 0.005) # np.sort(np.unique(np.hstack((np.arange(0.27, 0.5, 0.003), np.arange(0.284, 0.296, 0.0008))))) # np.arange(0.25, 0.5, 0.005) # # choose_hyperparams(valid_measures, eval_thresh_func) # None 
    ...
    #             rank_hyperparams(itertools.product(epochs_arr, learning_rate_arr, num_trees_arr, eval_thresh_func_arr, batch_size_arr), valid_measures)
            if(validation_flag):
                ...
                print('    Best hyperparms are: epochs = %d, learning_rate = %f, num_trees = %d, eval_thresh_func = %s, batch_size = %d, aggregator-type=%s, A_val_string=%s, feature_normalize=%s, varType=%s, A_regularizer=%s'\
            ...
            if('W_graphL' in self.task_core.feature_mode):
                ...
                _, _, _, _, prob_hat_test, W_hat_test = feature_extraction.test(X_test, y_test, self.task_core.target, train_test='testing', \
                                                                                    out_th=valid_eval_threshold, printing_flag=True, plotting_flag=Windows,\
                                                                                        clip_sizes = matlab_load_core.clip_sizes_test,\
                                                                                           sel_win_nums = matlab_load_core.sel_win_nums_test) # 
...
#                 num_trees = num_trees_arr[0]
    ...
    #             print('********')
                if(X_test is not None):
                    if('W_raw' in self.task_core.feature_mode ): 
                        prob_hat = clf.predict_proba(X_to_W(X_test, self.task_core.feature_mode, freq_band=freq_band))[:, 1]
                    ...
                    fine_eval_performance(y_test, prob_hat, sensitivity_score_target, th=valid_eval_threshold, printing_flag=True)  
                    print('********')
                else:
                    ...
                    while(not classif_loadbatch.end()):
                        ...
                        inn_prob = clf.predict_proba(X_to_W(classif_loadbatch.current_x(), self.task_core.feature_mode, freq_band=freq_band))[:, 1]
            ...
            X_total = matlab_load_core.X_train # np.concatenate((X_train, matlab_load_core.X_test), axis=0)
            y_total = matlab_load_core.y_train # np.concatenate((y_train, matlab_load_core.y_test), axis=0)
            clip_sizes_total = matlab_load_core.clip_sizes_train # np.concatenate((matlab_load_core.clip_sizes_train, matlab_load_core.clip_sizes_test), axis=0)
            dftest = None
            Delays = []
            FAs = []
            total_hours = 0
            matlab_engine = matlab.engine.start_matlab()
            for in_train_idx, in_test_idx in EU_online_fileNum_RollingWindow(matlab_load_core, self.task_core): 
...
#                 print('wind_idx train = ', wind_idx)
                ...
                in_X_test, in_y_test, sel_win_nums, conv_sizes, clip_sizes = \
                    inpython_online_wrapper(matlab_engine, matlab_load_core, in_test_idx, 'total')
                ...
                for epochs, learning_rate, num_trees, eval_thresh_func, batch_size, aggregator_type, A_val_string, multiClass_ratio, feature_normalize, varType, A_regularizer in hyperparams_arr:
                    side_adj_mat = side_adj_mat_arr[hypparamCounter]   
                    in_X_train, in_y_train = multiClass_undersampling(X_total[wind_idx, ...], y_total[wind_idx], multiClass_ratio)
                    ...
                    if('W_graphL' in self.task_core.feature_mode):
                        ...
                        feature_extraction.printing(self.task_core.adj_calc_mode)
                        _, _, _, _, prob_hat_train, _ = feature_extraction.test(in_X_train, in_y_train, self.task_core.target, train_test='training', printing_flag=False)
                    print('TRAINING')
                    fine_eval_performance(in_y_train, prob_hat_train, sensitivity_score_target, printing_flag=True)
                    break
                ...
                if('W_graphL' in self.task_core.feature_mode):
                    FAs, Delays, dftest, total_hours, _, _ = feature_extraction.test(in_X_test, in_y_test, self.task_core.target, train_test='testing', \
                                                                                            ...
                                                                                            Delays=Delays, df=dftest, total_hours=total_hours) 
        return 
