...
class train_test(Task):
    def in_W_graphL_run(self, hyperparams, in_train_measures=None, in_valid_measures=None):
        ...
        side_adj_mat = None if 'optimizedA' else (2*self.matlab_load_core.structural_inf.adj_means[0]-1) # 'optimizedA', 'fixedA_invCov'
        if(True):
...
#                                                                                 out_th = np.arange(0.27, 0.32, 0.008), printing_flag=True) 
    ...
    def in_W_Raw_run(self, hyperparams, in_train_measures=None, in_valid_measures=None):
        freq_band, num_trees = hyperparams
        clf = RandomForestClassifier(n_estimators=num_trees)
        if('W_raw' in self.task_core.feature_mode): 
            feature_trained = X_to_W(self.X_train[self.train_idx,...], self.task_core.feature_mode, freq_band=freq_band)
            clf = clf.fit( feature_trained, self.y_train[self.train_idx] )
            prob_hat_train = clf.predict_proba(feature_trained)[:, 1]
            if(self.validation_flag):
                prob_hat_valid = clf.predict_proba(X_to_W(self.X_train[self.valid_idx,...], self.task_core.feature_mode, freq_band=freq_band))[:, 1]
    ...
    def run(self):
        ...
        num_nodes, self.dimArray, self.matlab_load_core = data_load(self.task_core).run(None) # matlab_load_core.side_adj_mean
        ...
        if('fine' in self.task_core.TrainTest_mode):
            X_test = self.matlab_load_core.X_test
            y_test = self.matlab_load_core.y_test
        else:
            X_test = None 
            y_test = None 
...
#         A_val_string_arr = ['optimizedA' if side_adj_mat is None else 'fixedA_invCov' for side_adj_mat in side_adj_mat_arr] 
        A_val_string_arr = ['optimizedA'] # 'optimizedA', 'fixedA_invCov'
        self.mvAvg_winlen = 4 # sec2win(2)
        multiClass_ratio_arr = [-1]
        feature_normalize_arr = [ False]
        varType_arr = ['diag_repeated'] # 'diag_repeated', 'full_repeated', 'full'
...
#         freq_bands = [[0.1, 4], [4, 8], [8, 13], [13, 30], [30, 50], [70, 100]]
        ...
        if('50%-50%' in self.task_core.TrainTest_mode):
            ...
            for multiClass_ratio in multiClass_ratio_arr:
                self.X_train, self.y_train, self.matlab_load_core.clip_sizes_train = \
                            multiClass_undersampling(self.matlab_load_core.X_train, self.matlab_load_core.y_train, multiClass_ratio, self.matlab_load_core.clip_sizes_train)
...
#                             try:
                                _, in_train_measures, in_valid_measures = self.in_W_graphL_run(hyperparams, in_train_measures, in_valid_measures)
#                             except Exception as e:
#                                 continue
                    else:
                        
                        for hyperparams in list(itertools.product(freq_bands, num_trees_arr)):
                            _, in_train_measures, in_valid_measures = self.in_W_Raw_run(hyperparams, in_train_measures, in_valid_measures)
...
#                 print('-------')
    ...
    #             rank_hyperparams(itertools.product(epochs_arr, learning_rate_arr, num_trees_arr, eval_thresh_func_arr, batch_size_arr), valid_measures)
            ...
            if('W_graphL' in self.task_core.feature_mode):
                feature_extraction, _, _ = self.in_W_graphL_run(hyperparams_arr[best_hyper])
                _, _, _, _, prob_hat_test, W_hat_test = feature_extraction.test(X_test, y_test, self.task_core.target, train_test='testing', \
                                                                                    out_th=valid_eval_threshold, printing_flag=True, plotting_flag=Windows,\
                                                                                        clip_sizes = self.matlab_load_core.clip_sizes_test,\
                                                                                           sel_win_nums = self.matlab_load_core.sel_win_nums_test) # 
...
#                 num_trees = num_trees_arr[0]
    ...
    #             print('********')
                if(X_test is not None):
                    clf, _, _ = self.in_W_Raw_run(hyperparams_arr[best_hyper])
                    if('W_raw' in self.task_core.feature_mode ): 
                        prob_hat = clf.predict_proba(X_to_W(X_test, self.task_core.feature_mode, freq_band=hyperparams_arr[best_hyper][0]))[:, 1]
                    ...
                    classif_loadbatch = online_load(self.matlab_load_core)
                    total_steps = 0
                    prob_hat = None
                    y_true = None
                    while(not classif_loadbatch.end()):
                        classif_loadbatch.next()
                        inn_prob = clf.predict_proba(X_to_W(classif_loadbatch.current_x(), self.task_core.feature_mode, freq_band=all_valid_measures[best_hyper][0]))[:, 1]
                        ...
                        coarse_eval_performance(y_true, prob_hat, self.sensitivity_score_target, th=valid_eval_threshold, matlab_load_core=self.matlab_load_core,
                                                mvAvg_winlen=self.classification_core.mvAvg_winlen) 
                        total_steps += 1 
        else:
