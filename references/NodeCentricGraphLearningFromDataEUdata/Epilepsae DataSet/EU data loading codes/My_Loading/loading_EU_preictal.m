...
if ismac,
    ...
    root_dir='C:/Users/Nafiseh Ghoroghchian/Dropbox/PhD/EU data loading codes/SZR_ANT-master';
end
...
[soz_chans_bi, n_soz_chan] = selection_good_channels(soz_chans_bi, badchans);
fprintf('# of SOZ Channels (just good): %d\n',n_soz_chan);
[chans_bi, n_chan] = selection_good_channels(bipolar_labels, badchans);
...
%% Loop over chans
ch_loop = chans_bi; % soz_chans_bi;
% n_tpt_ct=zeros(size(ch_loop,1),1);
loop_files = file_info; % cli_szr_info;
loop_n_files = n_files; % n_szrs;
for cloop=1:size(ch_loop,1),
%for cloop=1:1,
    fprintf('Working on chan %s-%s\n',ch_loop{cloop,1},ch_loop{cloop,2});
%    for sloop=7:7,
    for sloop=1:loop_n_files,
        % See if this channel was part of soz for this szr
%         if ismember(soz_chans_bi{cloop,1},cli_szr_info(sloop).clinical_soz_chans) || ismember(soz_chans_bi{cloop,2},cli_szr_info(sloop).clinical_soz_chans)
            
            fprintf('Part of SOZ for Szr %d\n',sloop);
            
            % Read header
            full_data_fname=get_data_fullfname(sub_id,loop_files(sloop).fname); %loop_files(sloop).clinical_fname
%             loop_files(sloop).clinical_fname
            pat=bin_file(full_data_fname); 
            Fs=pat.a_samp_freq;
            if isempty(Fs),
                error('Could not find file: %s',loop_files(sloop).clinical_fname);
            end
            fprintf('FS=%f\n',Fs);
            if isempty(Fs),
            ...
            pat.a_channs_cell={ch_loop{cloop,1}}; % Channel to import
            %ieeg(1:n_chan,:)=pat.get_bin_signals([],[]);
            ieeg_temp1=pat.get_bin_signals(1,pat.a_n_samples);
            
            pat.a_channs_cell={ch_loop{cloop,2}}; % Channel to import
            ...
            %% Compute ictal-class
            % Clinical Szrs
            if ~isempty(loop_files(sloop).clin_szr_onsets_sec),
                [szr_class, targ_window] = class_generation(ieeg, loop_files(sloop), 'clin');
            end
            % Subclinical Szrs
            if ~isempty(loop_files(floop).sub_szr_onsets_sec),
                [szr_class, targ_window] = class_generation(ieeg, loop_files(sloop), 'sub');
            end
            
            %%
            fszr_onset_tpt=round(Fs*loop_files(sloop).clinical_onset_sec);
            fszr_offset_tpt=round(Fs*loop_files(sloop).clinical_offset_sec);
            ...
            if Fs>256,
                ...
                [time_dec, targ_win_dec, szr_class_dec] = Down_Sample(ieeg, Fs);
            else
...
%             targ_raw_ieeg_sec=time_dec(targ_raw_ieeg_tpts);
            ...
            outfname=fullfile(outdir,sprintf('%d_%s_%s_szr%d',sub_id, ...
                ch_loop{cloop,1},ch_loop{cloop,2}, ...
                loop_files(sloop).clinical_szr_num));
            szr_fname=loop_files(sloop).clinical_fname;
