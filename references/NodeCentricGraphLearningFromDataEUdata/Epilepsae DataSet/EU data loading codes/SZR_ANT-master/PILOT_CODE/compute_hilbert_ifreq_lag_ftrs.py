...
if len(sys.argv)==1:
    print('Usage: compute_hilbert_ifreq_lag.py decay_factor')
    exit()
if len(sys.argv)!=2:
    raise Exception('Error: compute_hilbert_ifreq_lag.py requires 1 argument: params.json')
...
ftr_root = os.path.join(ftrs_root, 'IFREQ_LAG' + str(decay_fact))
...
text_file.write('_ifreq_lag'+str(decay_fact)+'.npz\n')
text_file.close()
...
ftr_list=['BbandIF','DeltaIF','ThetaIF','AlphaIF','BetaIF','GammaIF','HGammaIF']
...
for sub in use_subs_df.iloc[:,0]:
    ...
    for szr_ct, szr_name in enumerate(use_szrs):
        ...
        # Trim norm
        dg.trimmed_normalize(hilb_ifreq,.4)
        ...
        if not os.path.isdir(ftr_path):
            os.mkdir(ftr_path)
        ftr_fname=os.path.join(ftr_path,szr_name+'_ifreq_lag'+str(decay_fact)+'.npz')
        print('Saving features to file %s' % ftr_fname)
        np.savez(ftr_fname, peri_ictal=peri_ictal, time_wind_sec=sgram_sec, ftrs=hilb_ifreq, ftr_list=ftr_list)
