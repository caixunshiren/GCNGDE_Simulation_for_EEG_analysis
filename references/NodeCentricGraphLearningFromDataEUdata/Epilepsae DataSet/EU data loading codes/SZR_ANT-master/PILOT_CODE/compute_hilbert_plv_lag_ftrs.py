...
if len(sys.argv)==1:
    print('Usage: compute_hilbert_plv_lag.py decay_factor')
    exit()
if len(sys.argv)!=2:
    raise Exception('Error: compute_hilbert_plv_lag.py requires 1 argument: params.json')
...
ftr_root = os.path.join(ftrs_root, 'PLV_LAG' + str(decay_fact))
...
text_file.write('_plv_lag'+str(decay_fact)+'.npz\n')
text_file.close()
...
ftr_list=['BbandPLV','DeltaPLV','ThetaPLV','AlphaPLV','BetaPLV','GammaPLV','HGammaPLV']
...
for sub in use_subs_df.iloc[:,0]:
    ...
    for szr_ct, szr_name in enumerate(use_szrs):
        ...
        print('use_elecs {}'.format(use_elecs))
        print('type(use_elecs) {}'.format(type(use_elecs)))
        hilb_phz_dif, hilb_sec = ief.bp_hilb_phz_dif(ieeg[use_elecs, :], Sf,
                                                     wind_len, wind_step, tpts_sec, bands, filt_order)
        # hilb_phz_dif, hilb_sec = ief.bp_hilb_phz_dif(ieeg[[onset_chan_id, ctrst_ids], :], Sf,
        #                                              wind_len, wind_step, tpts_sec, bands, filt_order)
        print('hilb_phz_dif {}'.format(hilb_phz_dif.shape))
        ...
        if n_ctrst==1:
            if decay_fact>0:
                ...
                for tpt in range(1,n_hilby_tpt):
                    hilb_phz_dif[:,tpt]=(1-alph)*hilb_phz_dif[:,tpt-1]+alph*hilb_phz_dif[:,tpt]
            ...
            hilb_plv=np.mean(hilb_plv,1)

        # Trim norm
        dg.trimmed_normalize(hilb_plv,.4)
        ...
        if not os.path.isdir(ftr_path):
            os.mkdir(ftr_path)
        ftr_fname=os.path.join(ftr_path,szr_name+'_plv_lag'+str(decay_fact)+'.npz')
        print('Saving features to file %s' % ftr_fname)
        np.savez(ftr_fname, peri_ictal=peri_ictal, time_wind_sec=hilb_sec, ftrs=hilb_plv, ftr_list=ftr_list)
