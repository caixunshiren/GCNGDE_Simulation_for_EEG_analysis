# This script does LOOCV to create an ensemble of classifiers (e.g., 8 patients produces 8 classifiers). It does the
# same thing as train_ensemble.py but it weights the data from all patients equally so that patients with lots of seizures
...
for sub_ct in range(n_train_subs):
    ...
    print('Sub #%d has %f observations' % (sub_ct,n_obs))
...
for C_ct, C in enumerate(try_C):
    ...
    for left_out_id in range(n_train_subs):
        ...
        model.fit(ftrs[subset_id,:], szr_class[subset_id], sample_weight=samp_wts[subset_id]) # Correct training data
