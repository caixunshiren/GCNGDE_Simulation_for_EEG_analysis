...
if len(sys.argv)==1:
    print('Usage: compute_hilbert_diff_ifreq_lag.py decay_factor')
    exit()
if len(sys.argv)!=2:
    raise Exception('Error: compute_hilbert_diff_ifreq_lag.py requires 1 argument: an integer >=0 specifying smoothing degreee')
...
ftr_root = os.path.join(ftrs_root, 'DIFREQ_LAG' + str(decay_fact))
...
text_file.write('_difreq_lag'+str(decay_fact)+'.npz\n')
text_file.close()


# Define frequency bands
bands=list()
bands.append((1,125)) # Broadband
...
ftr_list=['BbandDIF','DeltaDIF','ThetaDIF','AlphaDIF','BetaDIF','GammaDIF','HGammaDIF']
...
for sub in use_subs_df.iloc[:,0]:
    ...
    for szr_ct, szr_name in enumerate(use_szrs):
        ...
        _, hilb_ifreq, sgram_sec=ief.bp_hilb_mag(ieeg[onset_chan_id:onset_chan_id+1,:], Sf, wind_len, wind_step,
                                          tpts_sec, bands)
        ...
        if decay_fact>0:
            ...
            for tpt in range(1,n_hilby_tpt):
                hilb_ifreq[:,tpt]=(1-alph)*hilb_ifreq[:,tpt-1]+alph*hilb_ifreq[:,tpt]

        # Take temporal difference (i.e., y(t)=x(t+1)-x(t)
        hilb_ifreq_diff=np.zeros(hilb_ifreq.shape)
        print('hilb_ifreq.shape: {}'.format(hilb_ifreq.shape))
        hilb_ifreq_diff[:,1:]=np.diff(hilb_ifreq)

        # Trim norm
        dg.trimmed_normalize(hilb_ifreq_diff,.4)
        ...
        if not os.path.isdir(ftr_path):
            os.mkdir(ftr_path)
        ftr_fname=os.path.join(ftr_path,szr_name+'_difreq_lag'+str(decay_fact)+'.npz')
        print('Saving features to file %s' % ftr_fname)
        np.savez(ftr_fname, peri_ictal=peri_ictal, time_wind_sec=sgram_sec, ftrs=hilb_ifreq_diff, ftr_list=ftr_list)
