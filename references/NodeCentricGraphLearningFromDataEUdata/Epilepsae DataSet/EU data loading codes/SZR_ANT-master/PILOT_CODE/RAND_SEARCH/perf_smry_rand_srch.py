...
plt.figure(3)
plt.clf()
_=plt.errorbar(C_vals[C_sort_ids],np.mean(dat[:,C_sort_ids],axis=0),np.std(dat[:,C_sort_ids],axis=0))
n_subs=dat.shape[0]
for a_ct, a in enumerate(C_vals):
    _=plt.plot(np.ones(n_subs)*a,dat[:,a_ct],'o')
#plt.xticks(np.arange(len(perf['try_C'])),xlabs)
