...
for ensemble_ct in range(n_ensemble):
    ...
    print('Mean alpha=%f' % np.mean(alpha))
...
valid_class_hat=np.zeros(n_valid_wind)
for ensemble_ct in range(n_ensemble):
    valid_class_hat+=ensemble_wts[ensemble_ct]*(model_list[ensemble_ct].predict(valid_ftrs.T)*2-1)

print('Ensemble validation data performance:')
jive=(valid_class_hat>=0)==valid_class
