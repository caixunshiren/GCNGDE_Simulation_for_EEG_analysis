...
if len(sys.argv)==1:
    print('Usage: compute_szr_class_EU.py patient_id')
    exit()
if len(sys.argv)!=2:
    raise Exception('Error: compute_szr_class_EU.py requires 1 argument: patient_id')
...
ftr_root = os.path.join(ftrs_root, 'EU_SZR_CLASS', sub)
if not os.path.isdir(ftr_root):
    print('Creating directory %s' % ftr_root)
    pre_root=os.path.join(ftrs_root, 'EU_SZR_CLASS');
