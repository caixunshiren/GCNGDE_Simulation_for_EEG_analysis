...
def parse_input_data(data_dir, target, data_type, pipeline, gen_ictal=False):
    ...
    def process_raw_data(data_dir, target, data_type, with_latency):
        ...
        for segment in mat_data:
            data = segment['data']
#             data = segment['ictal_segment_1'] #I think this will work for one segment = we should change the kez in line +
...
#             print(transformed_data)
    ...
    data = process_raw_data(data_dir, target, data_type, with_latency=ictal)
...
#     tagged_durations = sorted(tagged_durations,  key=lambda x,y: key(y[1], x[1]))
    tagged_durations = sorted(tagged_durations, key=cmp_to_key(cmp_wrapper_1) ) #lambda x,y: cmp(y[1], x[1])
    ...
    chosen = sorted(chosen , key=cmp_to_key(cmp_wrapper_2))
#     chosen = sorted(chosen, cmp=lambda x,y: cmp(x[0], y[0]))
