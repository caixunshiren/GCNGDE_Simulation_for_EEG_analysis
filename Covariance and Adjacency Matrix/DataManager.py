...
sys.path.insert(1, parent_dir + '/Unsupervised GCN')
from GCN import *
...
class dataManager:
    ...
    def get_sim_matrix_from_model(self, model_dir):
        checkpoint = self.load_ckp(model_dir)

        # preprocess inputs
        A = torch.from_numpy(self.create_DAD(self.A_train)).float()
    ...
    # load pretrained sim matrix --- this is incomplete
    def load_flattened_sim_matrix(self, DIR, dm):
